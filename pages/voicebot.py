...
with col_right:
    ...
    """, unsafe_allow_html=True)
 # Panel header with title, button, and badge in one line
col1, col2, col3 = st.columns([4, 2, 2])  # Adjust width ratios

with col1:
    st.markdown(f"<h3 style='margin:0'>{escape(t('voice_response_title', st.session_state.language))}</h3>", unsafe_allow_html=True)
...
with col3:
    ...
    st.markdown(f"<div style='text-align:right'>{badge_text}</div>", unsafe_allow_html=True)
